...
app.service('UserService', function (FIREBASE_URL, $q, $firebaseAuth,
                                     $firebaseObject, $rootScope, $localstorage,
                                     $ionicPopup, $ionicAnalytics) {
    ...
    registerUser: function() {
      ...
      user.save().then(function() {
		...
		toggleFavorite: function (show) {
			...
			self.ensureFavorite();
			if (self.current.favorites[show.id]) {
		...
		addFavorite: function (show) {
			self.ensureFavorite();
			self.current.favorites[show.id] = show;
      $ionicAnalytics.track('Favorite Show', show);
		...
		removeFavorite: function (show) {
			self.ensureFavorite();
			self.current.favorites[show.id] = null;
